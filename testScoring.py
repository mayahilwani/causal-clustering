...
def plot_scoring(rn, gt, data):
    ...
    for variable_index in rn:
        ...
        curr_node = Node(normalized_vars[:, variable_index].reshape(recs, -1), globe_)
        #print(' node done.')
        # Define indices for the two sections of the data
        i_g1 = np.arange(5000)  # First 5000 data points
        i_g2 = np.arange(len(data) - 1000, len(data))  # Last 1000 data points
        ...
        globew = GlobeWrapper(Max_Interactions, log_results, verbose, dims=dim);
        score_all = globew.ComputeScore(hinge_count, interactions, sse, score, len(y), curr_node.min_diff,
        ...
        score_split = globew.ComputeScoreSplit(hinge_count1, interactions1, sse1, score1, rows1, hinge_count2,
...
def main():
    try:
        base_path = "C:/Users/ziadh/Documents/CausalGen-Osman/maya_data/experimentNEWflipshift1"
